...
type InventoryRow = { id: string; size: string; stock: number; price: number; };
type Variant = { id: string; color: string; image_url: string | null; inventory: InventoryRow[]; };

type InitialProductData = {
    id: string; name: string; sku_base: string;
    brand_id: string; collection_id: string; category_id: string;
    product_variants: {
        id: string; color: string; image_url: string | null;
        inventory: { id: string; size: string; stock: number; price: number; }[];
    }[];
};
...
export function ProductForm({ brands, collections, categories, initialData }: ProductFormProps) {
  ...
  const [selectedBrand, setSelectedBrand] = useState<string>(initialData?.brand_id || '');
  const [variants, setVariants] = useState<Variant[]>(
    initialData?.product_variants.map(v => ({...v, id: v.id, inventory: v.inventory.map(i => ({...i, id: i.id})) })) || []
  );
  ...
  const handleAddVariant = () => setVariants([...variants, { id: crypto.randomUUID(), color: '', image_url: '', inventory: [] }]);
  const handleRemoveVariant = (id: string) => setVariants(variants.filter(v => v.id !== id));
  const handleAddInventoryRow = (vId: string) => setVariants(variants.map(v => v.id === vId ? { ...v, inventory: [...v.inventory, { id: crypto.randomUUID(), size: 'S', stock: 0, price: 0 }] } : v));
  const handleRemoveInventoryRow = (vId: string, iId: string) => setVariants(variants.map(v => v.id === vId ? { ...v, inventory: v.inventory.filter(i => i.id !== iId) } : v));
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // El 'brand_id' no se guarda directamente en 'products', pero lo pasamos para la lógica de dependencias.
    // Lo añadimos aquí por si lo necesitamos en el futuro en la action.
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-12">
      {/* SECCIÓN 1: DETALLES BÁSICOS (CÓDIGO RESTAURADO) */}
      ...
      {/* SECCIÓN 2: VARIANTES (DISEÑO MEJORADO) */}
      <div className="bg-white p-8 rounded-2xl shadow-lg ring-1 ring-gray-900/5">
            ...
            {variants.map((variant, vIndex) => (
                <div key={variant.id} className="bg-white p-6 rounded-xl border border-gray-200/80">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-8 gap-y-6">
                        <div className="lg:col-span-1">
                            ...
                            <div className="mt-2 flex justify-center items-center h-40 w-full rounded-lg border border-dashed border-gray-900/25 bg-gray-50 p-2">
                                {variant.image_url ? ( <Image src={variant.image_url} alt={`Vista previa de ${variant.color}`} width={150} height={150} className="h-full w-full object-contain rounded-md" onError={(e) => { e.currentTarget.style.display = 'none'; }} /> ) : ( <div className="text-center"> <ImageIcon className="mx-auto h-12 w-12 text-gray-300" /> <p className="mt-2 text-xs leading-5 text-gray-600">Sin imagen</p> </div> )}
                            </div>
                        </div>
                    ...
                    <div className="mt-8">
                      ...
                      <div className="grid grid-cols-12 gap-x-4 mb-2 px-2 text-xs font-medium text-gray-500">
                          <div className="col-span-12 sm:col-span-4">Talla</div>
                          <div className="col-span-6 sm:col-span-3">Stock</div>
                          <div className="col-span-6 sm:col-span-3">Precio ($)</div>
                      </div>
                      <div className="space-y-3">
                        {variant.inventory.map(inv => (
                            <div key={inv.id} className="grid grid-cols-12 gap-x-4 items-center">
                                <div className="col-span-12 sm:col-span-4"><select name={`size-${inv.id}`} defaultValue={inv.size} className={formInputStyle}><option value="">Talla</option>{SIZES.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                                <div className="col-span-6 sm:col-span-3"><input type="number" name={`stock-${inv.id}`} placeholder="0" defaultValue={inv.stock} className={formInputStyle} /></div>
                                <div className="col-span-6 sm:col-span-3"><input type="number" step="0.01" name={`price-${inv.id}`} placeholder="0.00" defaultValue={inv.price} className={formInputStyle} /></div>
                                <div className="col-span-12 sm:col-span-2 flex justify-end sm:justify-center"><button type="button" onClick={() => handleRemoveInventoryRow(variant.id, inv.id)} className="text-gray-500 p-2 rounded-full hover:bg-gray-100 hover:text-red-600"><TrashIcon className="h-5 w-5"/></button></div>
                            </div>
                        ))}
                      </div>
                      <button type="button" onClick={() => handleAddInventoryRow(variant.id)} className="mt-4 inline-flex items-center gap-1.5 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50"><PlusIcon className="h-5 w-5"/>Añadir Talla</button>
                    </div>
                </div>
            ))}
        </div>
        <button type="button" onClick={handleAddVariant} className="mt-8 inline-flex items-center gap-1.5 rounded-md bg-indigo-50 px-3 py-2 text-sm font-semibold text-indigo-600 shadow-sm hover:bg-indigo-100"><PlusIcon className="h-5 w-5"/>Añadir Variante</button>
       </div>
